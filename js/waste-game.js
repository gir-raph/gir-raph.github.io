...
// TOUCH + DESKTOP SAFE GAME LOGIC (click item, then click bin)
...
let items = [];
let shown = 0;      // how many objects already shown
let score = 0;
const maxItems = 30;
...
function loadItem() {
  if (shown >= items.length) {
    endGame(false); // ran out of trials
    return;
  }
...
function findBin(name) {
  for (const bin in mapping) {
    if (mapping[bin].includes(name)) return bin;
  }
...
document.querySelectorAll('.bin').forEach(bin => {
  bin.addEventListener('click', () => {
    ...
    if (currentItem.dataset.type === bin.dataset.type) {
      score++;
    } else {
      score--;
      bin.style.border = '5px solid red';
      setTimeout(() => bin.style.border = '3px solid #334155', 300);
    }
